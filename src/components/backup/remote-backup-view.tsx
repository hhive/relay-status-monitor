...
export function RemoteBackupView() {
  ...
  async function runAction(kind: 'test' | 'run') {
    ...
    try {
      ...
      setMessage(kind === 'test' && typeof body.backups === 'number'
        ? `${body.message || '连接测试成功'}，识别到 ${body.backups} 个备份`
        : body.message || (kind === 'test' ? '连接测试成功' : '备份任务已完成'));
