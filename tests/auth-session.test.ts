...
test('admin session uses an isolated fixed JWT contract and a 7 day default', async () => {
  ...
  assert.equal((payload.exp as number) - (payload.iat as number), 7 * 24 * 60 * 60);
...
test('admin session cookie has hardened root-scoped attributes', async () => {
  ...
  attachAdminSession(response, 'signed-admin-session', 7 * 24 * 60 * 60, 'production');
