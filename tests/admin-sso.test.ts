...
test('admin sso resolves a 7 day default TTL with a 7 day maximum', () => {
  assert.equal(resolveAdminSsoConfig(validEnvironment).sessionTtlSeconds, 7 * 24 * 60 * 60);
  assert.equal(resolveAdminSsoConfig({
    ...validEnvironment,
    RSM_ADMIN_SESSION_TTL_SECONDS: String(7 * 24 * 60 * 60),
  }).sessionTtlSeconds, 7 * 24 * 60 * 60);

  for (const ttl of ['0', '1.5', String(7 * 24 * 60 * 60 + 1), 'not-a-number']) {
    assert.throws(() => resolveAdminSsoConfig({
      ...validEnvironment,
